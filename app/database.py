    ...
    with engine.begin() as conn:
        ...
        if "chat_messages" in table_names:
            ...
            if "image_urls_json" not in chat_columns:
                conn.execute(
                    text("ALTER TABLE chat_messages ADD COLUMN image_urls_json TEXT DEFAULT '[]'")
                )
        ...
        if "conversation_meta" in table_names:
            ...
            if "last_start_intent_at" not in meta_columns:
                conn.execute(
                    text("ALTER TABLE conversation_meta ADD COLUMN last_start_intent_at DATETIME")
                )
            if "last_start_intent_stage" not in meta_columns:
                conn.execute(
