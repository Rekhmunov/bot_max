...
def run() -> None:
    ...
    with TestClient(app) as client:
        ...
        assert "Диагностика медиа/дедупликации" not in admin_settings_page.text
        assert "Fallback (3+ фото)" not in admin_settings_page.text
        assert "Dedup сработал" not in admin_settings_page.text
